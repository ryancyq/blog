...
function toggleTheme(event) {
  const forceDark =
    event.target instanceof HTMLInputElement ? event.target.checked : !!event;
  document.documentElement.classList.toggle("dark", forceDark);
  localStorage.setItem("color-theme", forceDark ? "dark" : "light");
}
```
...
[JavaScript-free Responsive Navigation Menu guide]: posts/collapsible-navigation-menu-without-js
